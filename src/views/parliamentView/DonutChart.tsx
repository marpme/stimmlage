...
export const DonutChart = ({ data }: DonutChartProps) => {
  ...
  const coalitions = useMemo(
    () => buildCoalitions(sortedData, total),
    [sortedData, total],
  );
        ...
        <div className="flex flex-col items-center gap-0 py-6 px-4">
          ...
          <svg width={size} height={size} style={{ overflow: "visible" }}>
            <g transform={`translate(${cx},${cy})`}>
              ...
              >
                {sortedData.reduce(
                  (top, d) => ((d.value ?? 0) > (top.value ?? 0) ? d : top),
                  sortedData[0],
                )?.name ?? ""}
              </text>
            </g>
          ...
          {/* Party legend */}
          <div
            className="w-full mt-4 border-t border-rule pt-4 animate-fade-up"
                ...
                .map((d, i) => (
                  <div key={i} className="flex items-center gap-2 min-w-0">
                    <span
                      ...
                      style={{ background: d.data.color, opacity: 0.88 }}
                    />
                    <span className="text-xs text-ink truncate flex-1">
                      {d.data.name}
                    </span>
                    <span className="text-xs text-ink-tertiary tabular-nums ml-auto">
                      {pct(d.data.value ?? 0)}%
                    </span>
                  </div>
                ))}
          ...
          {coalitions.length > 0 && (
            <div
              className="w-full mt-5 border-t border-rule pt-4 animate-fade-up"
            ...
            >
                      ...
                      <div className="flex flex-col gap-2.5">
                        {tierItems.map((c, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              className={`flex flex-col gap-1 ${!c.viable ? "opacity-40" : ""}`}
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`text-[11px] tabular-nums flex-shrink-0 ${c.viable && hasMajority ? "text-accent font-semibold" : "text-ink-tertiary"}`}
                                >
                              ...
                              >
                                ...
                                />
                                <line
                                  x1={`${majorityMarkerPct}%`}
                                  y1="-1"
                                  x2={`${majorityMarkerPct}%`}
                                  y2="7"
              ...
              <div className="relative mt-1" style={{ height: 14 }}>
                ...
                <span
                  className="absolute text-[10px] text-ink-tertiary -translate-x-full"
                  style={{ left: "100%" }}
                >
                  ²⁄₃
                </span>
