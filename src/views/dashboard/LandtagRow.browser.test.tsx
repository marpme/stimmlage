...
describe("LandtagRow", () => {
  it("renders parliament name", () => {
    render(
      <MemoryRouter>
        <LandtagRow parliamentId="1" pollData={buildPoll()} index={0} />
      </MemoryRouter>,
    );
  ...
  it("shows leader percentage", () => {
    ...
    expect(document.querySelector(".tabular-nums")?.textContent).toMatch(/\d+\.\d+%/);
  });
  ...
  it("returns null when parliament not found", () => {
    const { container } = render(
      <MemoryRouter>
        <LandtagRow parliamentId="nonexistent" pollData={buildPoll()} index={0} />
      </MemoryRouter>,
    );
