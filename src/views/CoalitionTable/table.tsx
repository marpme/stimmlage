...
function formatDelta(value: number | null): {
  ...
  if (value === null || value === 0)
    return { text: value === 0 ? "0.0" : "—", positive: false, neutral: true };
  return {
    ...
    >
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-rule bg-rule/20">
            ...
            <th className="px-4 py-2.5 text-left text-[10px] font-semibold tracking-widest uppercase text-ink-tertiary">
              {t("coalition.poll")}
              <span className="ml-2 font-normal tracking-normal opacity-40">
                · {t("coalition.fivePercentBarrier")}
              </span>
            </th>
          </tr>
        </thead>
        <tbody>
          {sortedData.map((item) => {
            const belowHurdle =
              item.value < FIVE_PCT && item.name !== "Sonstige";
            ...
            return (
              ...
              >
                {/* Party name */}
                <td
                  className={`px-4 py-3 align-top pt-3.5 ${belowHurdle ? "opacity-35" : ""}`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="font-medium text-ink text-xs leading-tight">
                      {item.name}
                    </span>
                  </div>
                </td>
