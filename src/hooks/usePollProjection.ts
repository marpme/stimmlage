...
export function projectPollData(
  parliamentId: string,
  ...
  const projectTo = new Date(
    now.getTime() + PROJECT_DAYS * 24 * 60 * 60 * 1000,
  );
  ...
  for (const [name, dateMap] of partyDateSums) {
    const points: [Date, number][] = [...dateMap.entries()]
      .map(
        ([dateStr, { total, count }]) =>
          [new Date(dateStr), total / count] as [Date, number],
      )
...
export function usePollProjection(
  ...
  const now = new Date();
  return useMemo(
    () => projectPollData(parliamentId, pollData, now, isLight),
    [parliamentId, pollData, isLight, now],
  );
}
